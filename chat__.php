...
<body>
  <div class="wrapper">
    <div class="chat-logo">
	  ...
	  <!-- Edited syuffs here -->
        ...
        <link>
			<img onclick="displaySweetAlert(this.id)" id ="twitterBTN" src="./assets/twitter_logo.png" alt="twitter_logo">
		</link>
		<link>
			<a href="whatsapp://send?text= GET 1000 POINTS to Win Bird Drone. click https://www.whodoyouwanttofind.com to know more." data-action="share/whatsapp/share">
				<img onclick="" id ="whatsappBTN" src="./assets/whatsapp_logo.png" alt="twitter_logo" style="width:30px; height:30px">
			</a>
		</link>
        <link><img onclick="instagramShare()" id ="instagramBTN"  src="./assets/instagram_logo.png" alt="instagram_logo"></link>
        <link> <img data-toggle="modal" data-target="#myModal_2" id ="facebookBTN" src="./assets/facebook_logo.png" alt="facebook_logo" /></link>
      </div>
  ...
  <!-- shows the users point -->
  	 <!-- Modal -->
  <div class="modal fade" id="myModal_userpoints"  role="dialog" style="width:100%">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
		...
		<div class="row">
			<div class="col-md-6">  
				<img src="assets/IconBirdy.png" />
					
		
			</div>
			<div class="col-md-6">  
				<center>Your Total points:<br /><div class="pointsDiv"> </div> <h2> 
		...
		<center> <h2>GET 1000POINTS - WIN A BIRD DRONE </h2></center>
        </div>
        <div class="modal-footer">
          <button type="button" class="close_userpoint success" data-dismiss="modal" id="">close</button>
        </div>
      </div>
...
<!--	<button id="myBtn">Open Modal</button> -->
	...
	<div id="myModal_prediction" class="myModal_prediction">
	  ...
	  <div class="modal-content_prediction">
		...
		<h2 style="text-align:center">Predict with Nyra and Win Awesome Prizes..</h2><hr />
		<br />
		 <div class="row">
			<div class="col-md-3" style=""> <br /><br /><img src="assets/IconBirdy.png" style="" class="" /></div>
			
			<div class="col-md-5" style="margin: 0 auto;float:right">
		  ...
		  <div class="col-md-4" style="" class="center">
  ...
  <script>
	...
	*/
	setTimeout(function(){
        $('#myModal_prediction').fadeIn('slow');
		$('#myModal_prediction').style.display = "block";
    },5000);
...
<script>
 function take_screenshot(buttonClickedid, inviteBtn)
				...
				{	
					if(inviteBtn == "Invite"){
						//invite friends
						 ...
						 // LocalStorage("getReferralCode", 2);
					//console.log(localStorage.getItem('getReferralEmail'));
					var getReferralEmail = localStorage.getItem('getReferralEmail')
					//increament points here
					$.ajax({
								type: "POST",
								url: "increment_referral_ajax.php",
								data: {getReferralEmail:getReferralEmail},
	...
	function LocalStorage(varName, value){
		
		if (typeof(Storage) !== "undefined") {
							// Store
							localStorage.setItem(varName, value);
						} else {
							localStorage.setItem(varName, 0);
						}
	}
	...
	function instagramShare(){
			...
			swal({
					title: "Get 1000POINTS - WIN a Bird Drone",
				    text: "Download screenshot of your chat with Nyra! Upload to your IG page with the hashtag #whodoyouwanttofind",
					icon: "success",
					button: "Download"
				}).then((click) => {
					if(click){
						take_screenshot("instagramBTN", "");
...
<script>
 ...
 $(function() {
                ...
                if (localStorage.chkbx && localStorage.chkbx != '') {
                    ...
                    $('#email_2').val(localStorage.usrname);
                } else {
                ...
                $('#remember_me').click(function() {
                    ...
                    if ($('#remember_me').is(':checked')) {
                        // save email
                        localStorage.usrname = $('#email_2').val();
...
//display modal popup
	...
	//collect modal paramteres for the returning users
		...
		{
		 email = $("#email_2").val();
		 localStorage.emailSaved = $('#email_2').val();
		...
		$.ajax({
			...
			//	console.log(data);
				...
				username = dataArr[0];
				 localStorage.usernameSavedToBeUsedInTheReturningUser = username; //this a patch to bypass error of returning user not showing username 
				 
				var usernameForReturningUser = localStorage.getItem('usernameSavedToBeUsedInTheReturningUser');
				//console.log(ucheya);
				...
				else
				{
					...
					$('#myModal_userpoints').modal({
							 show: true,							 
							 backdrop: 'static',
							 keyboard: false,
						 
						 });
					$('<div>'+dataArr[2]+'</div>').appendTo('.pointsDiv');
			
					//setUserToGuest(dataArr[0]); //set user to guest if no username
					botWelcomeMessage(dataArr[0]);
					 //popup to check users counts
					 
					$('#close_userpoint').click(function() 
					{
						 $('#myModal_userpoints').modal('hide');
	...
	**/
    ...
    function AddText(msg) 
	{
		   ...
		   $.ajax({
			...
			//console.log(getUserDetailsFromEmail(email_2));
			
			console.log('i ama here', getUserName);
...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-117376949-2"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-117376949-2');
</script>
